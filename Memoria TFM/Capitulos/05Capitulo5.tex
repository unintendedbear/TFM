...
The process is again the same, with the difference that now it has been included \textit{Naïve Bayes} as a reference in all the groups of experiments. The results are displayed in Tables \ref{tab:repurl_unb_traintest} (a) and (b). With regard to the 60\% - 40\% partition of part (b), files could only be separated and created randomly. This is because, taking into account step 3 (see Figure \ref{fig:work_diagram_2}), when a certain log entry happens to be placed in the training, or testing, file, all the others which share the same \ac{URL} core domain, will follow to the same file, and therefore the percentages are lost.
